...
Guidelines and specifications for the final project for Module 4.
...
### Problem 4: Natural Language Processing 

This is the most open-ended project of all! If you choose to do an NLP project, your task will not only include building a model to classify text, but also will include data collection. Thus, because you will be procuring your own data, you'll have to clear your dataset with an instructor first. 

NLP projects are rich grounds for visualization and feature engineering, and as such, these should be _essential_ elements in your project. The evaluation component will be similar to what you've seen before -- you should select several metrics and justify why you are using them. Finally, if your model ends up being only "okay", tell us why! Discuss what it might be failing to capture, and possible reasons why certain texts are misclassified. This can be extremely interesting to explore, as we are trying to get a computer to understand humans, which is an incredibly complex task!

If you're intruiged by doing an NLP project, but aren't sure what to do, one example of an NLP classification task is classifying news articles by content (for example, political versus sports versus celebrity gossip). But there are many, may more options! You might find some inspiration [here](https://github.com/NirantK/awesome-project-ideas#text). If you're interested in doing an NLP project that's outside of classification, talk to an instructor about it for approval!
...
3. An **_"Executive Summary" PowerPoint Presentation_** that gives a brief overview of your problem/dataset, and each step of the data science process.
...
* The notebook should be well organized, easy to follow, and code is commented where appropriate.  
    * Additionally, the notebook should contain well-formatted, professional looking markdown cells explaining any substantial code. All functions should have docstrings that act as professional-quality documentation.  
...
The rubric for your chosen project can be found in this repo. Take care to look at the appropriate rubric! There is one for the time series, reccomendation system, and image classification projects, and other for the NLP project. Make sure you are aligning yourself with the correct one! And, as always, ask the instructors if any row is unclear or you'd like a bit more information. 
